...
public class ConnectionPool {
    // Variabili non più "final" né hardcoded, verranno riempite dal file .properties
    ...
    public static synchronized void init(int poolSize) throws SQLException {
        ...
        // --- INIZIO LETTURA FILE .PROPERTIES ---
        try (InputStream in = Thread.currentThread().getContextClassLoader().getResourceAsStream("database.properties")) {
            if (in == null) {
                throw new SQLException("File database.properties non trovato nel classpath (assicurati che sia in src/main/resources o nella cartella sorgente principale)!");
            }
            Properties props = new Properties();
            props.load(in);

            // Carichiamo i dati e appendiamo i parametri per il fuso orario di MySQL
            ...
            System.err.println("Errore fatale: Impossibile leggere le credenziali del database!");
            e.printStackTrace();
            throw new SQLException("Impossibile caricare database.properties", e);
        }
        // --- FINE LETTURA FILE .PROPERTIES ---
        ...
        try {
            Class.forName("com.mysql.cj.jdbc.Driver"); // Caricamento dei Driver per il DataBase
